...
class DataLoaderClass(object):
    def __init__(self, param_dic):
        """Load and return imputed data as PyTorch tensors."""
    ...
    # Load data
    def load_data(self, file_path, prefix, device):
        """Load and return imputed data as PyTorch tensors."""
        pred = PreprocessData(self.param_dic)
        data = pred.read_data(file_path, prefix).values
        # data = data.to(device)
        # file_path = os.path.join(data_path, data_type, f'all_caths_{data_type}_imputed.csv')
        # data = pd.read_csv(file_path).values
        return torch.tensor(data, dtype=torch.float32).to(device=device)
    ...
    # X_val = torch.tensor(val.values).float()
...
#         for traj in tqdm(self.encoded_data['traj'].keys()):
#             # bar.update()
...
#         self.transitions_head_pos = self.transitions_head_pos % len(self.transition_indices_pos_last) if len(self.transition_indices_pos_last)>0 else 0 # if there are no positive samples, then it is set to 1
#         self.transitions_head_neg = self.transitions_head_neg % len(self.transition_indices_neg_last) if len(self.transition_indices_neg_last)>0 else 0 # if there are no positive samples, then it is set to 1
