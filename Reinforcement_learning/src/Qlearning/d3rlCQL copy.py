...
class DQNImpl(DiscreteQFunctionMixin, QLearningAlgoImplBase):
    ...
    def __init__(
        ...
        self._compute_grad = (
            CudaGraphWrapper(self.compute_grad) if compiled else self.compute_grad
        )
...
class DiscreteCQLImpl(DoubleDQNImpl):
    ...
    def __init__(
        ...
        compiled: bool,
        device: str,
        parent: Optional["DiscreteCQLImpl"] = None,
    ...
    def compute_loss(
        self,
        batch: TorchMiniBatchExtended,
        q_tpn: torch.Tensor,
    ) -> DiscreteCQLLoss:

        if self.parent is not None and self.parent._impl is not None:
            with torch.no_grad():
                # Compute parent's Q-values for the current observations
                parent_q_values = (
                    self.parent._impl._q_func_forwarder.compute_expected_q(
                        batch.observations
                    )
                )

                treatment_ = parent_q_values[:, 1]
                ...
                # parent_action_q_values = parent_q_values.gather(1, batch.action_level_1.long().unsqueeze(-1)).squeeze(-1)

                # Determine the threshold (e.g., mean Q-value)
                # threshold = parent_action_q_values.mean()
                # Create a mask for entries with Q-values above the threshold
                # mask = parent_action_q_values >= threshold
                ...
                batch = TorchMiniBatch(
                    ...
                    next_observations=batch.next_observations[mask],
                    terminals=batch.terminals[mask],
                    intervals=batch.intervals[mask],
                    next_actions=(
                        batch.next_actions[mask]
                        if batch.next_actions is not None
                        else None
                    ),
                    returns_to_go=(
                        batch.returns_to_go[mask]
                        if batch.returns_to_go is not None
                        else None
                    ),
                    device=batch.device,
                )
        ...
        if self.use_uncertainty_loss:
            penalty = self.uncertainty_weight * penalty
            loss += penalty
...
@dataclasses.dataclass()
class DiscreteCQLConfig_H(LearnableConfig):
    ...
    alpha: float = 1.0
    parent_action_size: int = 2  # SAC1 action size
    # level_2_action_size: int = 8  # SAC2 action size
    parent_config: Optional["DiscreteCQLConfig_H"] = None
    use_attention_parent: bool = False  # Enable attention mechanism
    num_attention_heads_parent: int = 4  # Number of attention heads
    use_uncertainty_loss_parent: bool = False  # Use uncertainty penalty in critic loss
    uncertainty_weight_parent: float = 0.001  # Uncertainty penalty weight
    use_attention_cql: bool = False  # Enable attention mechanism
    num_attention_heads_cql: int = 4  # Number of attention heads
    use_uncertainty_loss_cql: bool = False  # Use uncertainty penalty in critic loss
    uncertainty_weight_cql: float = 0.0001  # Uncertainty penalty weight
    ...
    def create(
        ...
        parent = (
            self.parent_config.create(device, enable_ddp)
            if self.parent_config
            else None
        )

        return DiscreteCQL(self, parent=parent, device=device, enable_ddp=enable_ddp)
...
class DiscreteCQL(QLearningAlgoBase[DiscreteCQLImpl, DiscreteCQLConfig_H]):

    def __init__(
        self,
        config: DiscreteCQLConfig_H,
        device: DeviceArg = False,
        enable_ddp: bool = False,
        parent: Optional["DiscreteCQLImpl"] = None,
    ):
    ...
    def inner_create_impl(self, observation_shape: Shape, action_size: int) -> None:
        ...
        #  Build level_1 if it exists but is not initialized
        if self.parent and self.parent._impl is None:
            print("Parnt is being initialized...")
            self.parent.create_impl(
                observation_shape,
                self._config.parent_action_size,
            )
            assert (
                self.parent._impl is not None
            ), "ERROR: Parent _impl is None after creation!"
        ...
        q_funcs, q_func_forwarder = create_discrete_q_function(
            ...
            use_attention=self._config.use_attention_cql,  # Enable attention mechanism
            num_attention_heads=self._config.num_attention_heads_cql,  # Use 4 attention heads
        )
        ...
        self._impl = DiscreteCQLImpl(
            ...
            compiled=self.compiled,
            device=self._device,
            parent=self.parent,  # Pass level_1 instance  (NEW)
            use_uncertainty_loss=self._config.use_uncertainty_loss_cql,  # Use uncertainty penalty in critic loss
            uncertainty_weight=self._config.uncertainty_weight_cql,  # Uncertainty penalty weight
        )
