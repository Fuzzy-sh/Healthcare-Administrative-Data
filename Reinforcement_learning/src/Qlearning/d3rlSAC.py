...
# Types
##################################################################
# npt: Type annotation for a NumPy array with optional shape and dtype info.
...
# npt.DTypeLike is equivalent to all the valid inputs you can pass to np.dtype(...)
DType = npt.DTypeLike
...
structlog.configure(
    processors=[
        # If you bind_contextvars(user_id=123), every log line will include user_id=123.
        structlog.contextvars.merge_contextvars,
        # Adds a log_level key (like 'info', 'debug', etc.) to the log event dictionary.
        structlog.processors.add_log_level,
        
        # These three (set_exc_info, format_exc_info, StackInfoRenderer) are typically used together for best exception handling.
        # If you log with stack_info=True, this processor adds the current Python stack trace.
        structlog.processors.StackInfoRenderer(),
        # Looks for exceptions passed via exc_info=True and makes sure exc_info is a proper exception tuple.
        structlog.dev.set_exc_info,
        # If exc_info is present, it converts it into a formatted string (traceback).
        structlog.processors.format_exc_info,
        #  Adds a timestamp to every log message.
        structlog.processors.TimeStamper(fmt="%Y-%m-%d %H:%M.%S", utc=False),
        # Pretty-prints the final log message to the console (with colors, indentation, etc.).
        structlog.dev.ConsoleRenderer(),
    ],
)


# Creates a logger tied to the current module.
# __name__ resolves to the module's name (e.g., "my_app.module1").
# This helps identify where logs are coming from when reading output.


# Equivalent in the standard logging module to logging.getLogger(__name__)
LOG: structlog.BoundLogger = structlog.get_logger(__name__)


# Protocol is from typing and introduced in PEP 544.
# It enables structural subtyping (also known as "duck typing" with type checking).
# Classes don’t need to inherit from ModuleProtocol to be treated as conforming—
# if they implement the required methods, they match the protocol.
# ModuleProtocol is useful when:
# You want to accept any object that behaves like a module container (e.g., for training, logging, checkpointing), without forcing inheritance.
# You want to enable static type checking (e.g., with mypy) or interface validation in a clean, Pythonic way.


# Describes what a model module must provide to be considered loggable:
class ModuleProtocol(Protocol):
    # Return all PyTorch modules (e.g., for parameter inspection).
    def get_torch_modules(self) -> dict[str, nn.Module]: ...
    # Return gradients by name (for logging or debugging).
    def get_gradients(self) -> Iterator[tuple[str, Float32NDArray]]: ...


# any implementation (e.g., a model) must expose a .modules property that returns something following ModuleProtocol.
class ImplProtocol(Protocol):
    @property
    def modules(self) -> ModuleProtocol:
        raise NotImplementedError

# a learning algorithm must expose an .impl property (which may be None), pointing to its internal implementation (typically containing models and logic).
class AlgProtocol(Protocol):
    @property
    def impl(self) -> Optional[ImplProtocol]:
        raise NotImplementedError

# Marks any object that has a .save() method as something that can be checkpointed to disk.
class SaveProtocol(Protocol):
    def save(self, fname: str) -> None: ...

# write_params: logs hyperparameters.
# before_write_metric and after_write_metric: hooks to prepare and finalize metric writing.
# write_metric: logs an individual metric value.
# save_model: saves a model implementing SaveProtocol.
# close: flush/close the logger.
# watch_model: tracks parameters/gradients (useful for deep inspection or debugging).
...
# Used to create a LoggerAdapter for a given experiment.
...
# This class is the glue between protocols and actual logging.
class D3RLPyLogger:
    _algo: AlgProtocol
...
# Adds a timestamp to the experiment name.
# Uses the factory to instantiate a logger.
    def __init__(
        self,
        algo: AlgProtocol,
    ...
    # Saves the model and gracefully shuts down the logger.
    def save_model(self, epoch: int, algo: SaveProtocol) -> None:
    ...
    # Measures time spent in a block and logs it as a metric (e.g., time_train_step).
    @contextmanager
    def measure_time(self, name: str) -> Iterator[None]:
        ...
        try:
            ...
            # LOG.info(f"Timing metric logged: {name}")
    # Allows external access to the underlying adapter (useful for custom logging behavior).
    @property
...
#################################################################
# These define generic type variables, but they’re bounded, meaning:

# TConfig can only represent a subclass of SerializableConfig

# TDynamicConfig can only represent a subclass of DynamicConfig

# This ensures that when generics are used in class methods, they return an instance of the correct subclass, not just the base class.
...
# When you decorate a class with @dataclasses.dataclass(), Python will automatically generate:
# __init__() — constructor
# __repr__() — readable string representation
# __eq__() — equality comparison
# __hash__() — (if specified)
# __lt__, __le__, etc. — (if ordering is enabled)
# When you build config systems, APIs, or anything that loads/saves structured data, you often need to serialize objects to JSON (for storage or transfer) and deserialize them back.
...
class SerializableConfig:
    ...
    # @classmethod is a decorator that you use to define a method that:
...
# This line defines a new class named DynamicConfig that inherits from another class called
# SerializableConfig. This means DynamicConfig will have access to the methods and properties
# of SerializableConfig, unless they are overridden.


# The @staticmethod decorator indicates that the method get_type() is a static method. That means:
# It doesn't take self or cls as its first argument.
# It doesn't rely on the instance (self) or the class (cls) for anything.
# You can call it on the class itself without creating an instance: DynamicConfig.get_type()
...
# Purpose: Converts a sequence of floats (like a list) back into a NumPy array.
# Used for: Deserialization – turning raw data (e.g., from JSON or config) into a usable NumPy object.
...
# Purpose: Converts a NumPy array into a regular Python list (sequence of floats).
# Used for: Serialization – preparing the data to be saved, exported, or encoded (e.g., into JSON).
...
# Can optionally return a NumPy array (Optional[NDArray]).
# config is defined as import dataclasses_json.config,
# which is a decorator for configuring serialization and deserialization of dataclasses.
# The config function is used with metadata= in a dataclasses.field(...) to tell dataclasses-json:
# How to encode a complex value (e.g. NumPy array)
# How to decode it from JSON
# What field name to use in JSON (via field_name=...)
# Whether to exclude it if None, and more
def make_optional_numpy_field() -> Optional[NDArray]:
    # dataclasses.field is a numpy-specific field that can be used in a dataclass.
...
# Prevents assignment to fields after initialization
# Setting frozen=True makes the class immutable, just like namedtuple or frozendict.


@dataclasses.dataclass()  # changed forzen=True was removed
class ConfigMetadata:
    # A reference to a class (cls) (not an instance) that is a subtype of DynamicConfig.
    base_cls: type[DynamicConfig]
    # A function that takes a DynamicConfig instance and returns a dictionary.(Used for serialization.)
    encoder: Callable[[DynamicConfig], dict[str, Any]]
    # A function that takes a dictionary and returns a DynamicConfig or None
    decoder: Callable[[dict[str, Any]], Union[DynamicConfig, Optional[DynamicConfig]]]
    # A mapping from string names to config classes (subclasses of DynamicConfig).
    config_list: dict[str, type[DynamicConfig]]

    # add_config method to add a new config class to the config_list.
...
# This is a global registry mapping:
# A base DynamicConfig class →
# Its associated ConfigMetadata
# Used to store and look up metadata for different config types.
CONFIG_STORAGE: dict[type[DynamicConfig], ConfigMetadata] = {}


def generate_optional_config_generation(
    # base_cls is a class (usually a subclass of DynamicConfig) that all registered config types must inherit from.
    ...
    # Local Registry: Holds a mapping from a string type name (like "my_config") to a config class.
    CONFIG_LIST: dict[str, type[TDynamicConfig]] = {}

    # returning functions
    # register_config: to register new config classes

    def register_config(cls: type[TDynamicConfig]) -> None:
        # Asserts that the config class is a subclass of base_cls
    ...
    # make_field: to create an optional dataclass field with custom serialization
    ...
    ######################################################################
    # Converts a config object to a serializable dictionary.
    # Handles None as a special "type": "none" case.
    # Otherwise, serializes using the config’s get_type() and serialize_to_dict().
    ...
    # Takes a dictionary, extracts type and params
    def _decoder(dict_config: dict[str, Any]) -> Optional[TDynamicConfig]:
        ...
        # CONFIG_LIST[name] is a class (subclass of DynamicConfig)
        # .from_dict(params) creates an instance of that config class (SerializableConfig) from a dictionary
...
def asdict_without_copy(obj: Any) -> dict[str, Any]:
    # Ensures that the object is indeed a dataclass instance.
    assert dataclasses.is_dataclass(obj)
    # Retrieves a list of field definitions from the dataclass.
    fields = dataclasses.fields(obj)
    # Builds a dictionary:
    # Keys = field names
    # Values = direct attribute values from the object
...
# Cast to the dtype that is sent to the function.
...
def stack_observations(observations: Sequence[Observation]) -> Observation:
    if isinstance(observations[0], (list, tuple)):
        ...
        # np.stack(..., axis=0): Stacks them into a single NumPy array, creating a batch dimension at axis 0.
        # (np.array([1, 2]), np.array([0.1])), (np.array([3, 4]), np.array([0.2])), --> np.array([[1, 2], [3, 4]]) , np.array([[0.1], [0.2]])
        return [
...
# check if the type of values in array are the same as dtype
...
#  dataset components
##################################################################
# This class will generate data with the size and the type that we want.
...
class Signature:
    ...
    def sample(self) -> Sequence[NDArray]:
        ...
        # Generates a random array using np.random.random(shape)
        # Casts it to the correct dtype using .astype(dtype)
        return [
            np.random.random(shape).astype(dtype) for shape, dtype in zip(self.shape, self.dtype)
        ]


# If observations is a NumPy array, say of shape (32, 4, 4), it returns (4, 4) — that is, it strips the batch dimension.
...
# if only the type is within th np types 
...
class PartialTrajectory:
    ...
    # generating some observation based on the shape and dtype of the recieved dataset
    @property
    ...
    def get_transition_count(self) -> int:
        ...
        """
        # If the last terminal flag is True, it means the episode ended with a terminal state.
        # If it is False, it means the episode ended with a non-terminal state.
...
# x.shape = (3, 4)
# target.shape = (2, 3, 4)
# Then target.shape[1:] == x.shape, so this passes.
...
def add_leading_dims(x: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    ...
    # return x as (1,3,4) if target.shape is (2,3,4)
...
# ImplBase: the class that does the core work (like defining the model and gradient updates),
# LearnableConfig: the class that defines hyperparameters like learning rate, batch size, etc.
...
# ✅ Marks this protocol as one that can be checked at runtime.
# 📐 Ensures any class with the required interface will match this Protocol.

@runtime_checkable
class OptimizerWrapperProto(Protocol):
    # Requires that implementing classes expose an .optim property returning a torch.optim.Optimizer.
    @property
    def optim(self) -> Optimizer:
        raise NotImplementedError

    # Must provide a .state_dict() method (like PyTorch optimizers) that returns a mapping (e.g., for checkpointing).
    def state_dict(self) -> Mapping[str, Any]:
        raise NotImplementedError

    # Must provide a .load_state_dict() method to load optimizer state from a mapping (e.g., for resuming training).
...
# Fuzzy added, new        
class Checkpointer(Checkpointer_):
    def __init__(
        self,
        modules: dict[str, Union[nn.Module, OptimizerWrapperProto]],
        device: str,
        parent_checkpointer: Optional[Checkpointer_] = None,
    ):
        super().__init__(modules, device)
        self._parent_checkpointer = parent_checkpointer

    def save(self, f: BinaryIO) -> None:
        # Save child modules to buffer
        child_buffer = io.BytesIO()
        super().save(child_buffer)

        # Save parent modules if present
        parent_buffer = None
        if self._parent_checkpointer is not None:
            parent_buffer = io.BytesIO()
            self._parent_checkpointer.save(parent_buffer)

        # Combine into one dictionary
        torch.save(
            {
                "child": child_buffer.getvalue(),
                "parent": parent_buffer.getvalue() if parent_buffer else None,
            },
            f,
        )

    def load(self, f: BinaryIO) -> None:
        data = torch.load(f, map_location=map_location(self._device))

        # Load child modules
        super().load(io.BytesIO(data["child"]))

        # Load parent modules
        if self._parent_checkpointer is not None and data.get("parent") is not None:
            self._parent_checkpointer.load(io.BytesIO(data["parent"]))

    @property
    def parent_checkpointer(self) -> Optional[Checkpointer_]:
...
# Covariance allows subtypes to be accepted in a parent-type context.
# Covariance is mostly helpful for read-only or return-type-safe behavior. Think of it like:
# “You can substitute a more specific implementation if needed.”
...
# This is a base class for all implementation objects (like CQLImpl, DiscreteSACImpl, etc.).
# It uses ABCMeta to signal it’s an abstract class — you don't instantiate it directly.
...
class ImplBase(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        self._action_size = action_size
        self._modules = modules
        # Create a checkpointer for saving/loading model state.
        self._checkpointer = modules.create_checkpointer(device)
        self._device = device

     # Handles serialization of model weights.
    def save_model(self, f: BinaryIO) -> None:
        self._checkpointer.save(f)

    # f is a file-like binary object (e.g., from open("model.pt", "wb"))
    # These are used by LearnableBase.save_model() and load_model().
...
# The hyperparameters needed to initialize an algorithm (like SAC, CQL).
# A create() method that instantiates the algorithm.
# A system for registration so that it can be serialized and deserialized (for saving/loading models).
@dataclasses.dataclass()
# Inherits from DynamicConfig, a helper that supports serialization (.to_dict(), .to_json(), etc.).
...
# It's a generic class: takes in TImpl_co (the model/impl class) and TConfig_co (the config class).
# ABCMeta allows it to define abstract methods (@abstractmethod).
# It’s not meant to be used directly – you subclass it to implement real algorithms.
...
@dataclasses.dataclass()
class LearnableConfigWithShape_(DynamicConfig):
    observation_shape: Shape
    action_size: int
    config: LearnableConfig = make_learnable_field()
...
class LearnableConfigWithShape(DynamicConfig):
    ...
    # Optional parent config
    parent_config: Optional["LearnableConfigWithShape_"] = None

    def create(
        self, device: DeviceArg = False
    ) -> "LearnableBase[ImplBase, LearnableConfig]":
        # Create parent if present
        parent_algo = None
        if self.parent_config:
            parent_algo = self.parent_config.create(device)

        # Create child algo and pass parent if required
        algo = self.config.create(device)
        # Attach the parent if the config supports it
        if hasattr(algo, "set_parent") and parent_algo is not None:
            algo.set_parent(parent_algo)
...
class DiscreteMeanQFunctionForwarder(DiscreteQFunctionForwarder):
    ...
    def compute_error(
        ...
        try:
            ...
            raise RuntimeError(
                "Failed to convert actions to one-hot encoding. "
                "Check if actions are within the range of action size."
            )
...
def create_discrete_q_function(
    ...
    use_attention: bool,  # New flag for enabling attention
...
def asdict_as_float(obj: Any) -> dict[str, float]:
    ...
    for field in fields:
        ...
        if isinstance(value, torch.Tensor):
            assert value.ndim == 0, f"{field.name} needs to be scalar. {value.shape}."
...
from d3rlpy.algos.qlearning.torch.sac_impl import DiscreteSACImpl
...
class HierarchicalDiscreteSACImpl(DiscreteQFunctionMixin, QLearningAlgoImplBase):
    ...
    def __init__(
        ...
        compiled: bool,
        device: str,
        parent_sac: Optional["DiscreteSACImpl"],  # SAC1 instance  (NEW)
        ...
        super().__init__(
            ...
            modules=modules,
            device=device,
            # parent_sac=parent_sac,  # Pass parent SAC instance (if any) (NEW)
        )
        self._gamma = gamma
        self._q_func_forwarder = q_func_forwarder
        self._targ_q_func_forwarder = targ_q_func_forwarder
        self._target_update_interval = target_update_interval
        self.parent_sac = parent_sac  # Store parent SAC instance (if any) (NEW)
    ...
    def compute_target(self, batch: TorchMiniBatch) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            if self.parent_sac is not None and self.parent_sac._impl is not None:

                # Get SAC1's decision logits & actions
                # sac1_dist = self.sac1._modules.policy(batch.observations)
                sac1_dist = self.parent_sac._impl.policy(batch.observations)
    ...
    def inner_update(self, batch: TorchMiniBatch, grad_step: int) -> dict[str, float]:
        ...
        if self.parent_sac is not None:
            sac1_critic_loss = self.parent_sac._impl.update_critic(batch)
            sac1_actor_loss = self.parent_sac._impl.update_actor(batch)
...
@dataclasses.dataclass()
class HierarchicalDiscreteSACConfig(LearnableConfig):
    ...
    batch_size: int = 32
    gamma: float = 0.99
    n_critics: int = 1  # Number of Q-functions in the ensemble
    target_update_interval: int = 8000

    # initial temperature value for SAC
    initial_temperature: float = 1.0

    # Hierarchical RL (NEW)
    parent_sac_action_size: int = 2  # Parent SAC action size
    action_size: int = 8  # SAC2 action size

    parent_sac_config: Optional["DiscreteSACConfig"] = None
    use_parent_sac_filtering: bool = False

    use_attention_parent: bool = False  # Enable attention in parent SAC
    use_attention_sac: bool = False  # Enable attention mechanism

    num_attention_heads_parent: int = 4  # Number of attention heads in parent SAC
    num_attention_heads_sac: int = 4  # Number of attention heads in SAC
    ...
    def create(
        ...
        parent_sac = (
            self.parent_sac_config.create(device, enable_ddp)
            if self.parent_sac_config
            else None
        )
        return HierarchicalDiscreteSAC(
            self,
            device,
            enable_ddp,
            parent_sac=parent_sac,
            use_parent_sac_filtering=self.use_parent_sac_filtering,
            #    num_attention_heads_parent=self.num_attention_heads_parent,
            #    use_attention_sac=self.use_attention_sac,
    ...
    @staticmethod
    def get_type() -> str:
        return "hierarchical_discrete_sac"



class HierarchicalDiscreteSAC(QLearningAlgoBase[HierarchicalDiscreteSACImpl, HierarchicalDiscreteSACConfig]):
    def __init__(
        self,
        config: HierarchicalDiscreteSACConfig,
        device: DeviceArg = False,
        enable_ddp: bool = False,
        parent_sac: Optional["DiscreteSAC"] = None,  # SAC1 instance  (NEW)
        use_parent_sac_filtering: bool = False,  # SAC1 filtering flag  (NEW)
        ...
        self.parent_sac = parent_sac  # Store SAC1 instance  (NEW)
        self.use_parent_sac_filtering = (
            use_parent_sac_filtering  # Store SAC1 filtering flag  (NEW)
        )
        # self.use_attention = use_attention  # Store attention mechanism flag
        # self.num_attention_heads = num_attention_heads  # Store number of attention heads
        # self.use_uncertainty_critic_loss = use_uncertainty_critic_loss  # Store uncertainty flag
        # self.uncertainty_critic_weight = uncertainty_critic_weight  # Store uncertainty penalty weight
        # self.use_uncertainty_actor_loss = use_uncertainty_actor_loss  # Store uncertainty flag
        # self.uncertainty_actor_weight = uncertainty_actor_weight  # Store uncertainty penalty weight

    def inner_create_impl(
        self, observation_shape: Shape, action_size: int  # Default action size for SAC2
    ) -> None:
        """
        If SAC1 exists, its output is added to SAC2's input space.
        """
       
        #  Build parent_SAC if it exists but is not initialized
        if self.parent_sac and self.parent_sac._impl is None:
            print("parent_SAC is being initialized...")
            self.parent_sac.create_impl(
                observation_shape, self._config.parent_sac_action_size
            )
            assert (
                self.parent_sac._impl is not None
            ), "ERROR: parent_SAC _impl is None after creation!"
        ...
        print(self._config.parent_sac_action_size)
        # exit()
        # Create SAC2 Q-function, policy, and temperature parameter
        q_funcs, q_func_forwarder = create_discrete_q_function(
            observation_shape,
            self._config.action_size,  # Use SAC2 action size
            self._config.critic_encoder_factory,
            self._config.q_func_factory,
            n_ensembles=self._config.n_critics,
            device=self._device,
            enable_ddp=self._enable_ddp,
            use_attention=self._config.use_attention_sac,  # Enable attention mechanism
            num_attention_heads=self._config.num_attention_heads_sac,  # Use 4 attention heads
        )
        targ_q_funcs, targ_q_func_forwarder = create_discrete_q_function(
            observation_shape,
            self._config.action_size,  # Use SAC2 action size
        ...
        self._impl = HierarchicalDiscreteSACImpl(
            observation_shape=observation_shape,
            action_size=self._config.action_size,  # Use SAC2 action size
            modules=modules,
            q_func_forwarder=q_func_forwarder,
            targ_q_func_forwarder=targ_q_func_forwarder,
            target_update_interval=self._config.target_update_interval,
            ...
            compiled=self.compiled,
            device=self._device,
            parent_sac=self.parent_sac,  # Pass SAC1 instance  (NEW)
            use_uncertainty_actor_loss=self._config.use_uncertainty_actor_loss,  # Use uncertainty penalty in actor loss
            uncertainty_actor_weight=self._config.uncertainty_actor_weight,  # Uncertainty penalty weight
            use_uncertainty_critic_loss=self._config.use_uncertainty_critic_loss,  # Use uncertainty penalty in critic loss
            uncertainty_critic_weight=self._config.uncertainty_critic_weight,  # Uncertainty penalty weight
        )
...
# from d3rlpy.base import register_learnable

# register_learnable(HierarchicalDiscreteSACConfig)